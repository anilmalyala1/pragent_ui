...
export default function PRReviewAgent(){
  ...
  React.useEffect(() => {
    setLoadingPRs(true);
    axios.get(apiUrl('prs','/api/prs?owner=anilmalyala1&repo=pragent_api'))
      .then(r => {
        const list = r.data || [];
        setPrs(list);
        if (list.length) setSelectedPR(list[0]);
      })
      .catch(console.error)
  ...
  React.useEffect(() => {
    ...
    axios
      .get(apiUrl('prs', `/api/files/prs/anilmalyala1/pragent_api/${selectedPR.id}/contents`))
      .then(r => {
  ...
  async function runAIReview(){
    ...
    try {
      const res = await axios.post(apiUrl('prs', `/api/review/anilmalyala1/pragent_api/${selectedPR.id}`));
